...
export default function BatteryTestModal({ visible, onClose, sendCommand, voltage }: Props) {
 ...
 else if (rest >= 12.0) verdicts.push(` ${t('battery.verdicts.weak')} (${rest.toFixed(1)}V)`);
 ...
 else if (crank >= 9.0) verdicts.push(` ${t('battery.verdicts.crankLow')} (${crank.toFixed(1)}V)`);
 ...
 else if (charge >= 13.0 && charge < 13.5) verdicts.push(` ${t('battery.verdicts.regLow')} (${charge.toFixed(1)}V)`);
 else if (charge > 14.5) verdicts.push(` ${t('battery.verdicts.regHigh')} (${charge.toFixed(1)}V)`);
