 ...
 */
 ...
 const cleanCmd = command.replace(/\s+/g, '').toUpperCase();
 if (cleanCmd === '0902') { 
 const parsedVin = vehicleIdentityEngine.parseVinResponse(response);
 if (parsedVin) {
 useBluetoothStore.getState().setSensorData({ vin: parsedVin }); 
 }
 } 
 }
 ...
 else if (command.startsWith('03') || command.startsWith('07') || command.startsWith('0A') || command === '04') this.parseMode03Response(command, response); 
 }
