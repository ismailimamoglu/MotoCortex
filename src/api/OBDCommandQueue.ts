...
// MotoCortex v7.9.9 - Production-Grade High-Fidelity Command Queue Pipeline
...
import { assertHardwareGate } from '../core/security/CommandClassificationRegistry';
...
export function waitForELMPrompt(maxWaitMs: number = 1000, pollIntervalMs: number = 50): Promise<void> {  
   return new Promise<void>((resolve) => {  
       ...
       poller = setInterval(() => {  
           if (OBDCommandQueueInstance.hasPromptInBuffer()) {  
...
class OBDCommandQueue {  
   ...
   constructor() {  
       ...
       // Çözülen setLockGuard Metot Eşleşmesi
   ...
   add(command: string, timeoutMs?: number): Promise<string> {  
       ...
       if (this.blacklist.has(cleanCmd)) {  
           return Promise.reject(new Error('PID_BLACKLISTED_BY_UDS_GATE'));  
       }
       ...
       return new Promise<string>((resolve, reject) => {  
           CommandScheduler.add(command, priority, estimatedCost, timeoutMs)  
               ...
               .catch((err) => {  
                   // Propagate the specific rejection error from CommandScheduler (like SESSION_CANCELLED or TEST_CONNECTION_LOST)
                   // instead of overriding it when the session changes, so the active command's connection loss error is preserved.
                   reject(err);  
               });  
       });  
   }
   ...
   clear(error: Error = new Error('CONNECTION_LOST')) {  
       ...
       if (rejecter) { try { rejecter(error); } catch {} }
...
const OBDCommandQueueInstance = new OBDCommandQueue();  
export { OBDCommandQueueInstance };  
export default OBDCommandQueueInstance;